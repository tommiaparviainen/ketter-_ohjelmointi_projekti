...
namespace hellopello
{
    ...
    public class ContractBlock
    {
        ...
        public ContractBlock(int BlockId, string title, string content, string cat)
        {
            blockId = BlockId;
            Title = title;
            Content = content;
        ...
        public ContractBlock CopyBlock(int newId, string newTitle, string newContent, string newCategory)
        {
            return new ContractBlock(newId, newTitle, newContent, newCategory);
        }
    ...
    public class Contract
    {
        ...
        public void LinkBlockToContract(int contractId, int blockId, int sortOrder)
        {
            ...
            {
                connection.Open();
                string q = @"INSERT INTO contract_block (ContractId, BlockId, SortOrder)
                ...
                using (MySqlConnection connection = new MySqlConnection(MySqlCon))
                {
                    ...
                    using (MySqlDataReader reader = cmd.ExecuteReader())
                    {
                        while (reader.Read())
                        {
                            ...
                            blocks.Add(new ContractBlock(id, title, content, Category));
                        }
                    }
                }
                return blocks;
        ...
        class UI
        {
            ...
            private void CreateContractBlock()
            {
                ...
                Console.WriteLine("Enter block category:");
                string category = Console.ReadLine();


                // BlockId = 0, koska AUTO_INCREMENT luo sen
                ContractBlock block = new ContractBlock(0, title, content, category);
            ...
            private void CopyContractBlock()
            {
                ...
                ContractBlock original = new ContractBlock(id, "origTitle", "origContent", "origCategory");

                // Luo kopio
                ContractBlock copy = original.CopyBlock(0, "Copy of " + original.ToString(), "Copied content", "copied category");
            ...
            private void UpdateContractBlock()
            {
                ...
                int id = Convert.ToInt32(Console.ReadLine());
                ContractBlock block = new ContractBlock(id, "oldTitle", "oldContent", "oldCategory"); // pitäisi hakea DB:stä
            ...
            private void AddBlockToContract()
            {
                ...
                ContractBlock block = new ContractBlock(blockId, "title", "content", "category");

                contract.addBlock(block, 0);

                // Kutsu oikealla tavalla
                contract.LinkBlockToContract(contractId, blockId, 0);
                ...
                using (MySqlCommand cmd = new MySqlCommand(blocksQuery, connection))
                    {
                        ...
                        using (MySqlDataReader reader = cmd.ExecuteReader())
                        {
                            ...
                            if (!reader.HasRows)
                            {
                                Console.WriteLine("No blocks linked to this contract.");
                            }
                            while (reader.Read())
                            {
                                Console.WriteLine($"[{counter}] {reader.GetString(1)} ({reader.GetString(3)}, Public={reader.GetBoolean(4)})");
